...
from cosgcn.reshape import graphreshape
from cosgcn.deepgcn import adjPrecess, DeepGCN
from cosgcn.utils import csr_to_torch_sparse_tensor, set_device, normalize_adj_sym, eliminate_zeros_sparse, torch_sparse_to_dense
device = set_device()
class COSGCNTrainer:
    def __init__(self,
                 device=None,
    ...
    def fit_and_eval(self, features, labels, perturbed_adj, split_train, split_val, split_unlabeled, dataset_name='dataset', target_nodes=None):
        ...
        print(f"[COSGCN] Candidate pairs for structural updates: {len(cand_pairs)}")
        for ep in range(self.epochs):
            ...
            if patience <= 0:
                print(f"[COSGCN] Early stopping at epoch {ep}")
                break

        print("[COSGCN] Final evaluation with best model...")
        ...
        print(f"[COSGCN] Final acc: {final_acc:.4f}")
